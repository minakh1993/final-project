...
public class UpdateContactRequestResponseManager extends RequestResponseSetting {
	


	String updateContactPath="";
	...
	public boolean updateContact(PhoneRecord phoneRecord){
		try {
			...
			String requestBody = j.createJson(phoneRecord);
...
private HttpURLConnection connectionProperties() {
		...
		try {
			URL url = new URL(constant + IP + ":" + port + "/" + sharedPath + updateContactPath);
